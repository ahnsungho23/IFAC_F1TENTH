...
> 실차 YAML의 `auto_init_from_waypoints: true` 기본값은 유지합니다. 다만 gym 기본 스폰은
> 첫 IFAC waypoint와 약 18 m 떨어져 있으므로 `sim/run.sh mcl`은 SIM 역할에서만 자동 초기화를
> 끕니다. `f1sim -> mcl -> /initialpose -> global` 순서로 시작하십시오.
...
글로벌 플래너는 기동 시 설치된
`share/global_planning/data/<map_name>/global_waypoints.json`을 한 번 읽고 주행 중에는
...
>
> 검출기 기준맵에도 `F1_MAP`/`map_name`이 적용됩니다. SIM `ifac_track`은 설치된 v2
> `kinematic_localization/maps/ifac_track.yaml`을 `/local_planning/reference_map`에 올립니다.
> historical `map_kissmap_render.yaml`은 SIM에 사용하지 않습니다.
