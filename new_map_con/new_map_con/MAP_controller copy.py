...
class MAPController(Node):
    def __init__(self):
        ...
        self.state = "GB_TRACK"
        self.map_path = self.declare_parameter("map_path", "maps/oct28.csv").get_parameter_value().string_value
        self.sim = self.declare_parameter("sim", True).get_parameter_value().bool_value
        self.scan = None
        self.LUT_name = self.declare_parameter("LU_table", "SIM_linear").get_parameter_value().string_value
        self.steer_lookup = LookupSteerAngle(self.LUT_name, logger_info)
        self.t_clip_min = self.declare_parameter("t_clip_min", 0.5).get_parameter_value().double_value
        self.t_clip_max = self.declare_parameter("t_clip_max", 3.0).get_parameter_value().double_value
        self.m_l1 = self.declare_parameter("l1_distance", 3.0).get_parameter_value().double_value
        self.q_l1 = self.declare_parameter("l1_gain", 1.0).get_parameter_value().double_value
        self.speed_lookahead = self.declare_parameter("speed_lookahead", 1.0).get_parameter_value().double_value
        self.lat_err_coeff = self.declare_parameter("lateral_error_coeff", 1.0).get_parameter_value().double_value
        self.acc_scaler_for_steer = self.declare_parameter("acceleration_scaler_for_steering", 0.5).get_parameter_value().double_value
        self.dec_scaler_for_steer = self.declare_parameter("deceleration_scaler_for_steering", 0.5).get_parameter_value().double_value
        self.start_scale_speed = self.declare_parameter("start_scale_speed", 0.5).get_parameter_value().double_value
        self.end_scale_speed = self.declare_parameter("end_scale_speed", 0.5).get_parameter_value().double_value
        self.downscale_factor = self.declare_parameter("downscale_factor", 1).get_parameter_value().integer_value
        self.speed_lookahead_for_steer = self.declare_parameter("speed_lookahead_for_steering", 1.0).get_parameter_value().double_value
        ...
        self.create_subscription(PoseWithCovarianceStamped,'/amcl_pose',  self.car_state_cb, 10) # car position (x, y, theta)
        self.create_subscription(Odometry, '/frenet/odom',self.car_state_frenet_cb, 10) # car frenet coordinates
    ...
    def car_state_cb(self, data: PoseWithCovarianceStamped):
    ...
    def car_state_frenet_cb(self, raw: Odometry):
        ...
        if self.control_timer is None:
            try:
                self.control_timer = self.create_timer(1.0 / float(self.rate), self.main_loop)
            except Exception as e:
        ...
        try:
            ...
            s = frenet_msg.pose.pose.position.x = float(frenet_pos[0][0])
            d = frenet_msg.pose.pose.position.y = float(frenet_pos[1][0])
            vs = frenet_msg.twist.twist.linear.x = float(frenet_vel[0][0])
            vd = frenet_msg.twist.twist.linear.y = float(frenet_vel[1][0])
    ...
    def map_cycle(self):
        ...
        self.get_logger().debug(f"wp_safety: {self.waypoint_safety_counter}, rate: {self.rate}, sm_rate: {self.state_machine_rate}")
...
###########################################MAP CONTROLLER###########################################
    def main_loop(self, state, position_in_map, waypoint_array_in_map, speed_now, position_in_map_frenet, acc_now, track_length):
...
############################################MSG CREATION############################################
    ...
    def load_wpnts_from_csv(self, csv_path: str = None, downsample: int = 1) -> WpntArray:
        """
        Load waypoints from a CSV file and return a `WpntArray` message.

        Expected CSV columns (header):
            s, x_m, y_m, psi_rad, kappa_radpm, vx_mps, ax_mps2

        Parameters:
            csv_path: path to CSV file. If None, uses `self.map_path`.
            downsample: integer factor to downsample rows (1 = keep all)

        Returns:
            f110_msgs.msg.WpntArray populated with Wpnt messages.

        Notes:
            - Missing track-bound fields `d_left`, `d_right`, and `d_m` are set to 0.0 by default.
            - IDs are assigned sequentially starting at 0 based on the selected rows.
        """
        path = csv_path if csv_path is not None else getattr(self, 'map_path', None)
        if path is None:
            self.get_logger().error('No csv_path provided and self.map_path is not set')
            return WpntArray()
        ...
        # Basic column checks and flexible column name handling
        expected = ['s', 'x_m', 'y_m', 'psi_rad', 'kappa_radpm', 'vx_mps', 'ax_mps2']
        for col in expected:
            if col not in df.columns:
                self.get_logger().warning(f'Column "{col}" not found in CSV, missing values will be set to 0')

        wpnt_array = WpntArray()
        wpnts = []
        # iterate with downsampling
        rows = df.iloc[::max(1, int(downsample))]
        for idx, (_i, row) in enumerate(rows.iterrows()):
            wp = Wpnt()
            try:
                wp.id = int(idx)
            except Exception:
            ...
            # s may be labeled 's' in csv
            wp.s_m = float(row['s']) if 's' in row.index else 0.0
            wp.d_m = float(row['d_m']) if 'd_m' in row.index else 0.0
            wp.x_m = float(row['x_m']) if 'x_m' in row.index else 0.0
            wp.y_m = float(row['y_m']) if 'y_m' in row.index else 0.0
            wp.d_right = float(row['d_right']) if 'd_right' in row.index else 0.0
            wp.d_left = float(row['d_left']) if 'd_left' in row.index else 0.0
            wp.psi_rad = float(row['psi_rad']) if 'psi_rad' in row.index else 0.0
            wp.kappa_radpm = float(row['kappa_radpm']) if 'kappa_radpm' in row.index else 0.0
            wp.vx_mps = float(row['vx_mps']) if 'vx_mps' in row.index else 0.0
            wp.ax_mps2 = float(row['ax_mps2']) if 'ax_mps2' in row.index else 0.0

            wpnts.append(wp)

        wpnt_array.wpnts = wpnts
        self.get_logger().info(f'Loaded {len(wpnts)} waypoints from {path}')
        return wpnt_array
