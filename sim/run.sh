...
#   oppdet                                                    (optional opponent detector, 8)
#   stop | scratch
#
...
typeset -a PAT_SIM PAT_MCL PAT_GLOBAL PAT_LOCAL PAT_STATE PAT_WPNT PAT_CONTROL PAT_OPPDET
...
case "$role" in
  ...
  stop|clean|kill)                         # kill every node this stack owns, then exit
    ...
    kill_pattern "${PAT_OPPDET[@]}" "${PAT_CONTROL[@]}" "${PAT_WPNT[@]}" \
