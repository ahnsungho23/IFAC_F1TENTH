...
def generate_launch_description():
    ...
    # F1/F3 (2026-08-22) 실차 A/B 스위치 — local_planner_only 로 그대로 넘긴다.
    hold_republish_arg = DeclareLaunchArgument(
        'hold_republish_last_guidance', default_value='true',
        description='F1: 보류 게이트가 마지막 유효 회피 기하를 재검증해 계속 쓴다',
    )
    hold_min_clear_arg = DeclareLaunchArgument(
        'hold_min_clear_time_sec', default_value='0.0',
        description='F5: 곧 해소될 보류를 래치로 키우지 않는다 [s], 0 이면 끔',
    )
    stop_geometry_extend_arg = DeclareLaunchArgument(
        'stop_geometry_extend', default_value='true',
        description='F3: 잘린 정지 경로의 조향 기하를 원본으로 되돌린다',
    )
    ...
    local_planner = IncludeLaunchDescription(
        ...
        launch_arguments={
            ...
            'hold_republish_last_guidance': LaunchConfiguration(
                'hold_republish_last_guidance'),
            'stop_geometry_extend': LaunchConfiguration('stop_geometry_extend'),
            'hold_min_clear_time_sec': LaunchConfiguration('hold_min_clear_time_sec'),
        }.items(),
    )
