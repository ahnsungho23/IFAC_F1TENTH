...
5. **토픽 발행**: 계산된 스플라인 경로를 `/avoid_waypoints`(OTWpntArray)로 발행합니다. `wpnt_publisher`가 이 토픽을 구독해 글로벌 라인에 병합한 뒤 `/local_waypoints`로 내보내 제어부에 전달하고, 동시에 `state_machine`의 회피 판단 입력으로도 쓰입니다. (스탠드얼론 `/local_waypoints` 직접 발행은 `publish_standalone_local` 기본값 **false**로 비활성화되어, 제어 전달은 `wpnt_publisher` 경유로 일원화됩니다.)
...
| **Publish** | `/avoid_waypoints` | `f110_msgs/msg/OTWpntArray` | `state_machine` 회피 판단용 회피 스플라인 웨이포인트 |
| **Publish** | `/local_waypoints` | `f110_msgs/msg/WpntArray` | 스탠드얼론 직접 발행 (`publish_standalone_local`, 기본 **false** — 비활성) |
...
- `ot_waypoints_topic` (string, 기본값: `"/avoid_waypoints"`): 회피 웨이포인트(OTWpntArray) 발행 토픽명
