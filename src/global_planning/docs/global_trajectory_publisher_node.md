...
1. 시작 시 JSON 디렉터리를 다음 우선순위로 결정한다.
   ...
   - `output_base_dir` 기본값은 설치된 `share/global_planning/data`이다.
...
| 파라미터 | 기본값 | 설명 |
| --- | --- | --- |
| `output_base_dir` | 설치된 `share/global_planning/data` | JSON 번들 상위 디렉터리 |
...
정상 launch는 패키지 share의 설치 자산을 사용하므로 현재 작업 디렉터리에 의존하지 않는다.
`global_planning.launch.py`는 YAML의 `map_name`을 launch 인자로 덮어쓰며, 인자를
생략하면 `F1_MAP` 환경변수 또는 `map`을 사용한다. 외부에서 생성한 번들은
`map_path:=/absolute/bundle/directory`로 명시한다.

## 6. 실행 방법

1. ROS 2 Humble과 워크스페이스를 source한다.
...
별도로 생성한 번들을 처음부터 사용하려면 해당 디렉터리를 명시한다.

```bash
ros2 launch global_planning global_planning.launch.py \
  map_name:=forza_map map_path:=/absolute/path/to/forza_map
```

이 경우 `/absolute/path/to/forza_map/global_waypoints.json`이 시작 전에 존재해야 한다.

## 7. 확인 절차

1. 로그의 정확한 JSON 경로, waypoint 수, 마지막 `s`를 확인한다.
