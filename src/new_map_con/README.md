...
`obstacle_detector` 테스트를 위한 `/opponent_racecar/odom`을 발행한다. 차량(vehicle)과 시뮬레이터(simulator) 토픽 프로파일을 모두 지원한다.
...
`obstacle_detector` 테스트용으로, 에고보다 느린 속도(기본 0.8배)로 주행하며 `/opponent_racecar/odom`을 발행한다.
...
# 터미널 4: obstacle_detector (상대차 감지)
ros2 launch obstacle_detector obstacle_detector.launch.py simulator:=true
```

상대차는 raceline을 따라 순환하며, `obstacle_detector`가 LiDAR로 감지해 `/opp_obs`로 발행한다.
