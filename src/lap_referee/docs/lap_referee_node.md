...
f1tenth gym 브리지가 발행하는 latched collision 상태를 우선 사용하고, LiDAR 최소거리와
stuck 판정은 이전 브리지 또는 다른 시뮬레이터를 위한 보조 판정으로 유지한다.
...
2. `odom`/`scan`/`drive`/`collision`을 구독하며 50 Hz 타이머로 상태를 갱신한다.
...
6. 다음 조건 중 하나로 롤아웃을 종료한다.
   ...
   - `collision`: `/ego_racecar/collision`을 한 번이라도 수신하면 그 Bool만 사용한다.
     토픽을 전혀 받지 못한 환경에서만 최소 LiDAR 거리 `collision_scan_threshold`를 사용한다.
...
| 구독 | `collision_topic` | `std_msgs/msg/Bool` | `/ego_racecar/collision` | 시뮬레이터 latched collision 상태 |
| 발행 | `drive_topic` | `ackermann_msgs/msg/AckermannDriveStamped` | `/drive` | 종료 시 0속도 정지(선택) |

메시지는 모두 ROS 2 표준(`nav_msgs`/`sensor_msgs`/`std_msgs`) 및 차량 제어 표준
`ackermann_msgs`를 사용한다.
...
| `collision_topic` | 시뮬레이터 collision Bool 토픽 | `/ego_racecar/collision` |
