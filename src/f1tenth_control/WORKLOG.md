...
### 1. 🔴 출발 급발진 = 자율 미체결 중 속도 명령 와인드업 → engage 순간 계단

**bag 8개 전부 동일 패턴.** `control_map_node`는 `/drive_mode`를 구독하지 않아 **자기가 라우팅되고
있는지 모른다**. MANUAL/E-stop으로 서 있는 동안에도 `last_target_speed_` 램프가 계속 감긴다
(`publish_safe_stop()`에서만 0으로 리셋되는데 글로벌 경로가 있으면 그 경로를 안 탄다).

| bag | 정차 중 `/drive_autonomous` | engage 순간 ERPM | 모터전류 max | Vin 최저 |
|---|---|---|---|---|
| 193713 | 1.50 (최대 3.00) | 0 → **6348 한 스텝** | **62.4 A** | 11.7 V |
| 195937 | **3.00 (최대 3.98)** | 0 → 6348 | 61.4 A | 11.6 V |
| 213154 | 1.50 (최대 3.07) | 0 → 6348 | 61.0 A | 12.1 V |

**매 출발마다 `l_current_max`(60A)에 정확히 붙는다** — 출발 가속을 제한하는 게 제어기가 아니라
VESC 전류 상한 하나뿐이라는 뜻. VESC 쪽 완충도 없다(`s_pid_ramp_erpms_s` 21160 = 5.0 m/s²,
`s_pid_kp` 0.006 → 6348 ERPM 오차에 즉시 38A).

⚠️ **07-27의 `stall_guard_enable` false 전환이 이걸 악화시킨다.** 어제 명령이 정확히 1.50에
묶여 있던 게 그 가드(`stall_hold_speed`)였다. 끄면 정차 중 명령이 프로파일 속도(4.5)까지
자유롭게 감긴다 — 195937에서 이미 3.98 관측. `base_max_accel` 2.5는 감기는 *속도*만 늦출 뿐이다.

**조치 — bumpless transfer**: `/drive_mode`(`std_msgs/String`) 구독 신설. `autonomous`가 아니면
매 사이클 `last_target_speed_ = current_speed_`로 눌러둔다. engage 시점 명령이 실측과 같아져
계단이 사라지고, 그 뒤엔 `base_max_accel` 램프가 정상 가속을 만든다.
- ⚠️ **주행 중에는 아무 일도 안 한다** — 07-22에 금지한 일반 lead-clamp와 다르다. VESC 속도
  PID가 60A를 뽑는 데 필요한 명령 선행(~4.7 m/s)은 그대로 보존된다.
- ⚠️ `/drive_mode` 미수신·끊김(`drive_mode_timeout` 1.0s) 시 게이트 **자동 비활성** →
  시뮬(joy_teleop_monitor는 이 토픽을 발행 안 함)과 drive_mode_manager 미기동 시 구 거동 유지.
- 🔴 **개발 중 잡은 버그**: 런치 킥(8-c)이 `final_speed`가 아니라 `publish_speed`만 덮으므로,
  램프를 눌러놔도 킥이 켜져 있으면 발행값이 계속 `launch_boost_speed`(3.0)로 나갔다.
  미체결 중에는 킥도 돌리지 않도록 게이트(`!disengaged`)를 걸어 해결.

### 2. 🔴 "조향은 왼쪽인데 차는 오른쪽" = 조향 배선이 아니라 MCL pose 붕괴

**배선·부호·서보 변환은 정상**임을 bag으로 먼저 확인했다: `δcmd +0.410 → servo 0.2842 →
역환산 +0.410`, 잔차 중앙 0.0000. IMU 자이로 정지 바이어스도 +0.001 rad/s(0.06°/s)로 무시 가능.

진짜 원인 — `run_0727_195937` t=6.2~7.7s:
```
 t      idx    ct[m]    헤딩err     sinη     δcmd    ψ̇meas    vx
6.24     86    -0.03    +0.8°    +0.082   +0.106   +0.842   3.83
6.34     86    +0.90   -23.7°    -0.799   -0.280   +1.025   3.89  ← 0.1초에 ct 0.93m (횡속 9m/s, 불가능)
6.66     27    +0.94  +146.7°    +0.898   +0.227   -1.312   3.53  ← idx 86→27 텔레포트
7.66     32    +0.61  -173.0°    +0.431   +0.217   +1.224   0.66
```
MCL이 깨지며 `closest_idx`가 트랙 반대편을 오가고, **경로 접선과 차량 헤딩 오차가 +146°/−173°**
— 컨트롤러가 뒤를 향한 웨이포인트를 추종했다(이 bag은 주행 샘플의 **9.5%가 |오차|>90°**).
결과로 δ 명령이 0.2초마다 `−0.281 → +0.238 → −0.270`으로 부호를 뒤집었고, 조향↔요레이트
실행지연(~100–200ms)까지 겹쳐 **bag의 그 순간 명령과 그 순간 실측 요레이트가 반대로** 찍혔다.

**조치 — 방어막 2개**:
- `closest_idx_max_heading_err`(1.75rad=100°): 전역 재탐색에서 경로 접선이 차량 헤딩과 이 각도
  이내인 후보만 고려. 후보가 전무하면 게이트를 포기하고 무제한 스캔으로 폴백(재획득 불능 방지).
- `idx_jump_confirm_dist`(2.0m)/`idx_jump_confirm_cycles`(5): 한 사이클(20ms)에 물리적으로
  가능한 이동은 v·dt(8m/s에서 16cm)뿐. 그보다 먼 점프는 연속 5사이클 유지될 때만 채택하고,
  보류 중에는 조향을 직전값으로 홀드 + `pose_suspect_speed`(1.5)로 감속.
- 🔴 **오탐 버그**: 추적기를 0으로 되돌리는 자리(로컬 배열 교체, 글로벌 최초/범위이탈 재획득)에서
  비교 기준을 무효화하지 않아, 팀 플래너의 191점 풀랩 재발행마다 최대 17m 점프로 오판했다.
  시뮬 110초에 **107건** 발동 → 무효화 추가 후 **0~2건**.

**부수 발견 — 좌우 조향 비대칭**: `servo_min 0.2703/servo_max 0.6363`, gain −0.4463, offset 0.4672
→ 좌 최대 +0.441 rad인데 **우 최대 −0.379 rad**(vesc_driver가 클립). `run_0727_203040`에서 실제로
servo 0.6502가 발행돼 잘렸다. `steer_authority_ratio` 캡이 `MAX_STEERING_ANGLE`을 좌우 대칭으로
가정하므로 **우선회에서 낙관적**이다 — 서보 범위 실측 후 조정 필요(미해결).

### 3. 🔴 L1 횡가속 분모가 틀려 있었다 (조향 명령 최대 +70% 과대)

pure pursuit 법칙은 `a_lat = 2·v²·sin(eta)/L_실제`인데, 목표점은 `walk_forward`가 경로 **호 길이**
`L1_distance`만큼 전진해 고르므로 `|목표점−차량| != L1_distance`다. 그런데 코드는 명목
`L1_distance`를 분모로 썼다.

07-27 bag 실측 비율(`|목표점−차량| / L1_distance`): 중앙 **1.06~1.31**, p95 최대 **1.72**.
원인은 ① 웨이포인트 이산화(호 길이 누적이 목표를 최대 1점 간격 초과) ② 차량의 경로 오프셋.
→ 분모가 그만큼 작게 들어가 **횡가속 명령이 과대**하고, **경로에서 벗어날수록 = 복귀가 필요한
바로 그 순간에 더 심해진다.**

**조치**: 이미 계산돼 있던 `L1_norm`을 분모로 사용(하한 `t_clip_min`). `l1_use_actual_distance`
=false면 구 거동(즉시 롤백).

### 검증

**단위 시나리오 4종** (결정적 주입, 원형 트랙 R=6m):
| | 시나리오 | 결과 |
|---|---|---|
| A | `/drive_mode` 미발행 + 정차 | 명령 4.00 m/s = **구 거동 유지**(시뮬 호환) ✅ |
| B | `manual` + 정차 | 명령 **0.00 m/s** ✅ |
| C | `manual`→`autonomous` | 첫 명령 0.08, 사이클당 최대 증분 0.081 m/s = **계단 없음** ✅ |
| D | pose 12m 텔레포트 | 조향 +0.0908 **홀드** 후 5사이클 뒤 재획득 ✅ |

**L1 분모 A/B**(횡오차별 조향 명령): 이론값과 정확히 일치.
경로 위(오차 0)에서 old 0.09774 → new 0.09077. R=6m·L1=1.4m·간격 0.1885m에서 호길이 누적이
idx 8(현 1.508m)에서 멈추고 실제 현 길이 1.5045m → 비율 1.075 → 0.09774/1.075 = **0.0909** ✅

**시뮬 폐루프 A/B** (ifac_track, `/initialpose` 시드로 MCL 전역초기화 편차 제거, 각 2런 110초):

| 지표 | OLD (2런) | NEW (2런) | 변화 |
|---|---|---|---|
| 랩타임 mean | 18.23 / 18.27 | 18.28 / 18.30 | +0.04s (**+0.2%, 노이즈 수준**) |
| 크로스트랙 mean | 0.113 / 0.114 | 0.109 / 0.109 | **−4%** |
| 크로스트랙 max | 0.246 / 0.245 | 0.232 / 0.234 | **−5%** |
| 조향 \|mean\| | 0.1345 / 0.1335 | 0.1316 / 0.1315 | −2% |
| **조향 포화율(>0.40)** | 8.2 / 8.1% | 6.0 / 6.1% | **−26%** |
| **조향 부호 반전율** | 2.66 / 2.70 /s | 2.08 / 2.00 /s | **−24%** |

랩타임은 사실상 무변화(비시드 런에선 오히려 OLD 18.44 vs NEW 18.30로 역전 — 노이즈).
실질 이득은 **추종 정확도 개선 + 조향 포화·채터 대폭 감소**로, 계통적 과대 조향을 제거한
결과와 정확히 일치한다.

### 4. MCL 파라미터 — 시뮬은 이 변경을 검증할 수 없다(중요)

젯슨 `mcl_config.yaml`의 모션 노이즈가 **시뮬 튜닝값 그대로**였다. 주석에 `# increased for
simulation`이 붙은 채 실차에서 돌고 있었다:
```yaml
motion_dispersion_x: 0.15          # increased for simulation
motion_dispersion_theta: 0.25      # increased for simulation
```
코드상 이건 **30Hz 업데이트마다 파티클별로 더해지는 가우시안**이고 `noise_factor = 1+v/8`
(최대 2.0)이 곱해진다. 3 m/s에서 σ_θ = 0.25×1.375 = **0.34 rad(20°)를 초당 30번** 주입 —
순수 확산만 초당 1.9 rad. σ_x 0.15m는 8 m/s에서 한 스텝 이동(0.27m)의 55%다. 둘 다 비물리적.
→ **x 0.15→0.08, theta 0.25→0.10** 적용(젯슨, 백업 `mcl_config.yaml.bak.20260728`).

**🔴 시뮬 검증 시도와 그 실패를 기록해 둔다 — 같은 삽질 반복 방지.**
시뮬 폐루프에서 `/pf/pose/odom` vs gym ground truth로 MCL 오차를 직접 재봤다:

| 조건 | MCL err mean | 점프 |
|---|---|---|
| 현재값 (0.25/0.15/delay 3.0) | 0.1379 | 204 |
| 제안값 (0.05/0.05/delay 1.0) | 0.1385 | 205 |
| + scan 340ms 프리즈 주입, 현재값 | 0.1401 / 0.1401 (재현 2회) | 211 / 192 |
| + 프리즈, 제안값 | 0.1402 | 169 |
| + 프리즈 + **odom 열화**(요레이트 2.5배·속도게인 4%·노이즈), 현재값 | 0.1361 | 185 |

**전부 0.135~0.140으로 구분이 안 된다.** 이유: 시뮬 MCL은 odom 입력이 **ground truth**라
모션모델이 완벽하고, 잔여 오차 0.14m는 발행단 EMA 스무딩 지연이 지배한다(alpha=0.05+
min(1,v/2)·0.4 → v>2에서 0.45, 30Hz에서 ~40ms 지연 × 2.5 m/s ≈ 0.1m). odom을 실차 수준으로
열화시켜도 파티클 1000개 + 깨끗한 맵이면 여전히 수렴한다.
⚠️ **한 번 −45% 개선으로 보인 런이 있었으나 재현 2회에서 아웃라이어로 확인됐다**(0.2554 vs
0.1401/0.1401). 단일 런으로 판단하지 말 것.

→ **결론: 이 변경은 시뮬에서 "무회귀"만 확인됐고(랩타임·크로스트랙·조향 전부 동일),
개선 여부는 실차에서만 판정 가능하다.** 실차 A/B 절차는 아래.

#### 🔴 `delay_compensation_factor`는 값이 아니라 **클램프**가 문제다 (미적용, 팀 확인 필요)

처음엔 3.0 → 1.0을 제안했는데 코드를 다시 보니 그게 정답이 아니다. 3.0은 "스캔 전송+큐+처리"
총 지연이 처리시간의 ~3배라는 경험적 튜닝일 수 있다. 진짜 문제는 곱해지는 대상이다:
```cpp
double longitudinal_displacement = current_velocity_ * mcl_processing_time_ * DELAY_COMPENSATION_FACTOR;
```
`mcl_processing_time_`은 **CPU 스톨 때 같이 폭증한다.** 07-27 bag의 300ms 시스템 프리즈면
900ms 외삽 = 3.3 m/s에서 **3m 순간이동**. 즉 3.0이 상시 틀린 게 아니라 병리적 상황에서만
폭주한다. 올바른 수정은 값 변경이 아니라 **외삽량 상한**이다(정상 구간에선 무동작):
```cpp
// particle_filter.cpp ~1089
const double comp_dt = std::min(mcl_processing_time_ * DELAY_COMPENSATION_FACTOR, 0.05);
double longitudinal_displacement = current_velocity_ * comp_dt;
double angular_displacement      = current_angular_vel_ * comp_dt;
```
정상 처리 5~10ms × 3 = 15~30ms라 0.05s 상한은 **평소 절대 안 걸리고** 300ms 스톨만 자른다.
⚠️ **팀 패키지(`particle_filter_cpp`)라 적용하지 않았다.** 팀 합의 후 넣을 것.

#### 실차 MCL A/B 절차 (bag만 있으면 판정 가능)
1. 같은 코스를 두 번 주행하며 `f1rec`로 bag — 한 번은 백업 config, 한 번은 새 config.
2. `/pf/pose/odom`의 **스텝 점프**를 센다: `Δpos > 12cm` 이고 `> 2.5·v·Δt + 5cm` 인 샘플 수,
   그리고 `|Δyaw| > 12°/step` 수. 07-27 기준값은 런당 위치점프 4~33건 / 헤딩점프 0~10건.
3. ⚠️ **정차 중 LiDAR 차폐 구간은 제외**하고 셀 것(아래 참고). 차를 든 채로 돌아간 필터의
   발산을 파라미터 탓으로 오독하기 쉽다.

#### 🔴 운용 규칙 — 내려놓고 3~5초 기다린 뒤 A (코드 수정 0, 효과 큼)

07-27 bag의 pose 점프는 두 갈래인데 **한쪽은 순수 운용 문제**다.

| bag | pose 점프 | scan 공백>80ms | 점프가 프리즈 ±0.35s 내 | 정차 중 <30cm 빔 비율 |
|---|---|---|---|---|
| 193713 | 11 | 13 | **9 (82%)** | 0.0% |
| 194454 | 15 | 10 | **14 (93%)** | 64.1% |
| 195937 | 15 | 5 | **12 (80%)** | 66.3% |
| 202538 | 4 | **0** | 0 | **67.6%** |
| 203040 | 33 | **0** | 0 | **79.1%** |
| 213154 | 17 | **0** | 0 | **75.6%** |

- **갈래 A(19:37~19:59)**: `/odom`·`/scan`·`/imu`·`/sensors/core`가 **동시에 337~360ms 정지**하는
  시스템 스톨. 점프의 80~93%가 그 ±0.35s 안이다. 이 세 bag엔 `/drive_mppi`가 10Hz로 살아
  있었고, MPPI를 뺀 21:31 bag은 **프리즈 0건**이다 — 07-27의 MPPI 제거는 실제로 효과가 있었다.
- **갈래 B(20:25 이후)**: 프리즈가 0건인데도 점프. 이 bag들은 **정차 중 FOV의 67~79%가 30cm
  이내로 막혀 있다**(빔 729~852/1081, −135°~+96°). 차를 들거나 사람이 둘러싼 상태다.
  주행 중 중앙은 0.0~4.3%라 상시 장애물은 아니다.
  MCL은 그동안 계속 돌고, `z_short: 0.01`이라 "예상보다 훨씬 가까운 측정"에 사실상 확률 0을
  준다 → 전 파티클 가중치 붕괴 → 발산. **그 상태로 내려놓자마자 A를 누르면 재수렴 전에 출발한다.**

→ **차를 내려놓은 뒤 최소 3~5초 기다렸다가 A.** 가능하면 매번 `/initialpose`로 리셋.
  (헤드리스 시뮬 A/B에서도 `/initialpose` 시드를 넣자 MCL 전역초기화 실패로 인한 런 낭비가
  사라지고 랩타임 재현성이 18.23/18.27로 확 올라갔다 — 같은 원리다.)

### 5. 출발 급발진 — VESC 램프는 이제 "안전망"이지 주 대책이 아니다

`s_pid_ramp_erpms_s` **21160 → 12700**(= 3.0 m/s²)로 `vesc_mcconf.xml` 갱신.
⚠️ **XML 수정만으로는 차가 안 바뀐다 — VESC Tool로 플래시해야 한다.**

근거: 위 3-1의 bumpless transfer가 들어가면 명령이 0에서 `base_max_accel`(실차 2.5 m/s²)로
램프하므로 **VESC 램프는 더 이상 구속조건이 아니다**(2.5 < 5.0). 그래도 21160은 사실상 램프
해제 상태라, 컨트롤러 재시작·조이스틱 펀치·상류 노드 오동작 등으로 계단이 들어오면 그대로
60A를 때린다. 12700이면 컨트롤러(2.5)보다는 위여서 가속을 안 깎으면서 상한은 3.0 m/s²로 묶인다.
...
`servo_min`/`servo_max`는 servo **0.4533 중심 ±0.1830**(= ±0.41 rad)으로 계산된 값이다.
그런데 이후 **기계적 센터를 맞추며 트림 offset이 0.4672로 의도적으로 이동**했고
servo 범위는 갱신되지 않았다. 그래서 실제 가동각이:

| 방향 | 한계 servo | 실제 δ |
|---|---|---|
| 좌 (δ>0) | 0.2703 | +0.441 rad (여유) |
| 우 (δ<0) | 0.6363 | **−0.379 rad** (−7.6% 잘림) |

`run_0727_203040`에서 실제로 servo 0.6502가 발행돼 `vesc_driver`의 `servo_limit_.clip()`에
잘린 것이 관측된다.
...
| | 구값 | 신값 | δ |
|---|---|---|---|
| `servo_min` | 0.2703 | **0.2798** | +0.4199 rad (+24.06°) — travel **축소**라 무조건 안전 |
...
- 컨트롤러: `MAX_STEERING_ANGLE` 상수 하나 → **`max_steering_left`/`max_steering_right`
  파라미터로 분리**. 클리핑은 좌우 따로, **곡률 조향 권한 속도 캡·갭팔로워는 둘 중 작은 쪽**
  (큰 쪽을 쓰면 우선회에서 낙관 오류). 실차 런치 기본 0.42/0.42, **시뮬은 0.41/0.41 유지**
  (차량 모델이 대칭 — 하드웨어 특성을 시뮬 기준선에 옮기지 않는다).
- ⚠️ **두 변경은 반드시 한 쌍으로 움직인다.** 컨트롤러만 올리면 `vesc_driver`가 조용히 자르고
  컨트롤러는 "꺾었다"고 착각한다 → 요레이트 카운터스티어와 조향 권한 캡이 전부 낙관적이 된다.
- ⚠️ **우측 +0.0183 확장은 물리 검증 미완.** 첫 주행 전 바퀴 띄우고 풀락 좌/우를 한 번씩
  넣어 서보 스톨음·링키지 걸림이 없는지 확인할 것. 걸리면 `servo_max`를 낮추고
  `max_steering_right`를 같은 값으로 맞춘다. `tools/servo_range_probe.py`가 그 안전 탐색용
  (0.005씩 올리며 매 스텝 확인).

### ⚠️ 이 세션에서 발견한 워크스페이스 이슈 (제어 파트 밖)

- `~/2026_IFAC`에 **미해결 머지 충돌 2건**: `src/global_planning/launch/global_planning.launch.py`
  (주석 한 줄짜리 — 이것 때문에 **글로벌 플래너 런치가 SyntaxError로 아예 안 떴다**. 시뮬을
  돌리려고 해소했고 원본은 스크래치패드에 백업), `offline_trajectory_generator/gui_params.yaml`
  (손대지 않음). **팀에 확인 후 커밋할 것.**
- 이 랩탑에 **`F1_MAP` 환경변수가 없다** — 없으면 global_planning이 `map`을 찾아 실패한다
  (젯슨에만 반영됨, [[f1map-name-unification]]). `export F1_MAP=ifac_track` 필요.
- `global_trajectory_publisher_node`의 `map_dir`이 **상대경로**라 cwd가 `~/2026_IFAC`이어야 한다.
- 시뮬에서 `control_mppi_node`가 매번 **SIGABRT(exit -6)로 죽는다**(3런 3/3). 시뮬 런치엔 아직
  MPPI가 남아 있어 별도 조사 필요.
- `~/2026_IFAC` 사본의 `control_map_node.cpp`가 repo보다 앞서 있었다(`l1_distance` 0.6,
  `prebrake_decel` 1.8, `max_speed` 6.5, `min_speed` 2.5 — 전부 런치가 덮어쓰는 declare 기본값).
  **전체 복사가 아니라 patch로 반영**해 보존했다.

---

## 2026-07-27 — bag analyzer 4종 추가: 속도 구간별 가감속 · 속도 범위 타일 · 계기판 클러스터 · 랩 타임

팀원 요청(속도 구간별 가감속)과 대시보드·랩타임 요청을 반영. 배포 URL 동일:
...
### 4. 랩 타임 (계기판 타이밍 스트립 + 랩 리스트)
- **랩 검출 2방식**: 글로벌 경로가 있으면 최근접 웨이포인트 인덱스가 끝→처음으로 **감기는 순간**을
  결승선으로 본다(컨트롤러가 트랙을 보는 방식과 같고 경로 이탈에 견딤). 없으면 출발점 재통과 폴백.
  두 방식 모두 "트랙 후반부를 실제로 지났나"를 확인 — 결승선 근처 채터링·MCL 튐으로 랩이 세지면
  랩타임이 통째로 거짓이 된다.
- **클러스터 상단 타이밍 스트립**(F1 타이밍 타워 관례): `LAP n/N` · 현재 랩 경과(재생 따라 실시간) ·
  베스트(보라) · **Δ 베스트**(앞서면 초록/뒤지면 주황) + 랩 진행도 바.
  Δ는 베스트 랩의 **같은 트랙 지점** 경과시간과 비교하는 실제 레이싱 텔레메트리 방식
  (웨이포인트 인덱스를 진행도로 써서 보간).
- **랩 리스트**: 막대 길이 ∝ 랩타임, 베스트는 보라 + `BEST`, 나머지는 베스트 대비 `+격차`.
- ⚠️ **n번 통과 = n−1랩**. 첫 통과 이전(아웃랩)은 시작 지점이 트랙 어디였는지 알 수 없어 계측 불가 —
  "랩 수가 하나 적다"는 오해가 가장 잦은 지점이라 설명을 화면에 박아뒀다.
- 단위 검증(합성 데이터, 랩타임 10/8/12/9/11초 원형 주행): 두 방식 모두 `[8,12,9,11]` 정확히 복원,
  베스트 지목 정확. **반 바퀴만 → null**, **결승선 앞 정지 채터링 600샘플 → null**(가짜 랩 안 만듦).
