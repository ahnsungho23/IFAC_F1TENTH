...
def declare_common_args(
    ...
    return [
        ...
        # 2026-08-26: 0.9 → 1.3. bag rosbag2_2026_08_26-12_51_34 재생 결과, 이 값은
        # κ_L1 의 **구조적 상한**을 정한다: sin η 가 ±1 로 포화하면 κ_L1 = 2/l1_min_denom
        # 이고, U1 클램프의 v_grip 바닥은 √(MLA·margin·l1_min_denom/2) 이다.
        #   0.9 → κ_L1 상한 2.22, v_grip 바닥 1.87 m/s   (실측 최대 κ_L1 2.222 = 상한 포화)
        #   1.3 → κ_L1 상한 1.54, v_grip 바닥 2.25 m/s
        # 정상 코너링 비용은 **0**이다 — 클린랩 4개에서 L1_norm 최소가 1.42 m 라 1.3 은
        # 한 사이클도 구속하지 않는다(자율 전체로도 0.8%). 경로가 갱신되지 않아 목표점이
        # 차량 위로 무너진 구간(L1_norm 0.32 m)에서만 작동하는 발산 방지 바닥이다.
        # ⚠️ 1.5 부터는 클린랩 1.1%/자율 3.3% 를 구속하기 시작하고, 1.8 은 19.6% 라 못 쓴다.
        DeclareLaunchArgument(
            'l1_min_denom', default_value='1.3',
            description='L1 횡가속 분모 하한 [m] (목표점이 차량에 붙었을 때 발산 방지). '
                        't_clip_min과 무관하게 튜닝. κ_L1 상한 = 2/이 값'
        ),
        DeclareLaunchArgument(
        ...
        # 2026-08-26 재스윕(bag rosbag2_2026_08_26-12_51_34, 클린랩 4개, alpha 0.15 고정):
        #   margin  예산    구속%   재현율   랩당Δt
        #   0.95    7.41    20.3    103%*   +0.024      (*148건보다 많이 잡음 = 과보수)
        #   1.00    7.80    16.2    96.6%    기준       ← 유지
        #   1.05    8.19    13.1    82.4%   -0.047
        # 1.05 는 0.047 s 를 벌지만 재현율을 96.6% → 82.4% 로 떨어뜨린다 = 안전망을 파는 거래.
        # 1.00(예산 7.8)이 옳은 자리인 근거: 클린랩 자이로 실측 a_lat 이 p90 7.46 / p95 8.07 /
        # p99 9.05 로, 7.8 초과가 이미 시간의 6.68%(랩당 0.74 s)다. 예산이 실측 그립 벽
        # (마찰원 분석 8~9 m/s²) 바로 아래에 놓여 있다. 올리면 클램프가 일을 멈춘다.
        DeclareLaunchArgument(
            'grip_speed_clamp_margin', default_value='1.0',
            description='속도 예산 = 마진 없는 MLA × 이 값. 위 주석의 재스윕 표 참고 — '
                        '1.0 유지가 기본, 1.05 는 재현율 82%로 떨어져 권장하지 않음'
        ),
        # 2026-08-26: 0.05 → 0.15. bag rosbag2_2026_08_26-12_51_34 의 /debug/l1_lookahead 에서
        # κ_L1 을 전 구간 재구성(로그 대조 검증: 356.529s 0.444 vs 로그 0.455)한 뒤 이 필터만
        # 재생해 스윕한 결과다. 클린랩 4개(11.05~11.11 s, 간섭 없음) 기준:
        #
        #   alpha  τ[s]   구속%  탈출구속%  정밀도  재현율   랩당Δt
        #   0.05   0.50   23.1     36.6    29.5%   96.6%    —
        #   0.10   0.25   18.5     26.6    35.9%   96.6%   -0.052
        #   0.15   0.17   16.2     17.8    40.4%   96.6%   -0.068   ← 무릎
        #   0.20   0.12   15.5     15.7    41.3%   96.6%   -0.075
        #   0.30   0.08   14.9     14.6    41.9%   96.6%   -0.082
        #
        # 🔑 **재현율이 alpha 와 무관하게 96.6% 로 고정**이다(실측 a_lat>7.8 m/s² 148건 중
        #    143건을 0.5 s 안에 예고). 공격은 즉시(if k>filt: filt=k)라 alpha 는 해제만
        #    건드리기 때문이다. 그래서 이 값을 올리는 건 안전을 팔지 않는다 — 오히려
        #    헛발동이 152회 줄어 정밀도가 29.5% → 40.4% 로 오른다.
        # 왜 필요했나: 0.05(τ 0.5 s ≈ 4 m/s 에서 2 m)면 진입에서 잡은 최대 곡률을 탈출까지
        #    물고 간다. 탈출 구속 시점의 filt/κ_L1(현재) 가 p50 1.52 / p90 4.23 — 지금 없는
        #    곡률을 1.5~4배로 들고 있었다. 그때 차는 이미 레이스라인보다 0.76 m/s 느렸다
        #    (진입 구속 시점은 -0.04 m/s = 정당한 개입).
        # ⚠️ 0.30(τ 0.08 s ≈ 0.3 m)까지 가면 필터가 사실상 없어져 κ_L1 노이즈가 속도 명령에
        #    그대로 실린다. 추가 이득 0.014 s 를 위해 감수할 이유가 없다.
        DeclareLaunchArgument(
            'grip_speed_clamp_release_alpha', default_value='0.15',
            description='요구 곡률 해제 필터 계수 (제한 진입은 즉시, 해제만 τ≈0.17 s). '
                        '올려도 재현율 불변 — 탈출 구간의 낡은 곡률만 빨리 놓는다'
        ),
