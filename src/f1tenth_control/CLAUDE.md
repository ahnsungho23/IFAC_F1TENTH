...
- `control_map_node`의 나머지 파라미터는 전부 코드 내 `declare_parameter` 기본값(별도 YAML
  미연결). **`max_speed` 하나만 예외로 런타임 파라미터 콜백을 수용**(2026-08-15 —
  map_creator가 장애물 회피 글로벌 라인 스왑 직후 `swap_max_speed_mps`로 상한을 내리는 용도.
  양의 유한 double만 수락). **그 외 전부 생성자에서 1회만 읽음**(파라미터 콜백 없음) —
  값을 바꾸려면 노드 재시작 필요, 런타임 `ros2 param set`은 무효. 조정 경로에 따라
  3그룹으로 나뉨:
...
| 파라미터 | 기본값 | 설명 |
|---|---|---|
| `max_speed` | **8.0**(real) / 12.0(sim) | 직선 최고속도 캡 [m/s]. 곡률 제한은 코너에서만 걸리므로 직선 상한은 이 값이 유일하다. real 8.0 = ERPM 상한(바퀴 ~9 m/s)의 89% (2026-07-30 5.0→8.0 상향). ⚠️ 2026-08-15부터 **런타임 변경 수용** — map_creator가 장애물 라인 스왑 직후 `swap_max_speed_mps`(기본 7.0)로 내린다 |
...
| `l1_offset` / `l1_speed_gain` | **0.4** / 0.3 | L1 = `l1_offset`[m] + v·`l1_speed_gain`[s]. 2026-07-30 `l1_gain`/`l1_distance`에서 개명(구 이름 호환 shim은 08-06 제거 — 새 이름만 먹는다). 🔴 **2026-08-13 0.70 → 0.4 하향** — `l1_offset`은 속도와 무관한 상수라 **저속 추종오차의 바닥을 직접 정한다**(2 m/s에서 L1이 1.30 → 1.00 m). 근거는 ②-m. ⚠️ 전 속도에 걸리므로 레이스 페이스의 감쇠 여유(ω_n·τ)도 같이 깎인다 — 레이스 조건 재검증 전까지는 저속 세션에서만 확인된 값이다 |
...
⚠️ **아직 실차 검증 전이다.** 그리고 `l1_offset`은 전 속도에 걸려 레이스 페이스의 감쇠 여유도
깎는다(4 m/s에서 ω_n·τ 0.39 → 0.46, ②-f의 건전 대역 0.29~0.44 상단). **저속 세션에서 효과를
확인한 뒤 레이스 페이스를 따로 재검증할 것.**
