...
: rclcpp::Node("static_obstacle_map", options)
{
  ...
  RCLCPP_INFO(
    get_logger(),
    "static_obstacle_map started (confirmed=%s, dynamic=%s, visualization=%s, frame=%s, "
    "association=%.2fm, edge_confirm=%d, edge_tolerance=%.2fm, "
    "max_diagonal=%.2fm)",
    confirmed_static_obs_topic_.c_str(), dynamic_obs_topic_.c_str(),
...
void StaticObstacleMapNode::declareParameters()
{
  ...
  declare_parameter<std::string>(
    "visualization_topic", "/static_obstacle_map/markers");
  declare_parameter<std::string>("frame_id", "map");
  declare_parameter<std::string>(
    "visualization_marker_namespace", "persistent_static_obstacles");
  ...
  std::shared_ptr<std_srvs::srv::Empty::Response> response)
{
  ...
  RCLCPP_INFO(get_logger(), "Persistent static memory reset; removed %zu obstacle(s).", removed);
  publishVisualization();
}
...
void StaticObstacleMapNode::publishVisualization()
{
  ...
  auto markers = buildObstacleMarkers(
    header, memory_.obstacles(), marker_config_);
