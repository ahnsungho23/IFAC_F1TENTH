...
- `AVOID`: ego가 회피 경로 tail에서 글로벌 라인에 합류하면, 간섭 중일 때 `CRUISE`,
  아니면 `GLOBAL`로 복귀합니다. 또한 `/avoid_waypoints`가 비거나 갱신이 끊기고 ego가 마지막
  유효 경로 tail에 도달한 상태가 확인 시간 동안 지속되면 경로 단절로 판단해 같은 방식으로
  복귀합니다. 마지막 회피 waypoint 속도가 0 이하라 정상 합류가 거부되는 경우에는
  `/static_obs`의 전방 ego corridor가 비어 있는 상태가 1초(파라미터) 지속될 때만
  `GLOBAL(0)`으로 복귀합니다. 검출 토픽이 stale이면 안전을 위해 복귀하지 않습니다.
...
- AVOID 상태에서 전용 경로가 무효하면 `global_fallback` 정책으로 글로벌 전방 구간을
  발행합니다.
...
| 구독 | `/static_obs` | `f110_msgs/msg/ObstacleArray` | Reliable + Volatile, 정적/임시 정적 장애물 전방 안전 확인 |
...
| `avoid_path_stale_timeout_sec` | `0.5` | 회피 경로 갱신 중단 판정 시간 |
| `avoid_path_exhaustion_sec` | `0.5` | 경로 단절 상태 연속 확인 시간 |
| `avoid_path_exhaustion_s_gap_tol_m` | `0.75` | 마지막 유효 경로 tail 도달 허용 거리 |
| `static_obstacles_stale_timeout_sec` | `0.3` | 정적 장애물 입력 stale 판정 시간 |
| `stopped_path_clear_sec` | `1.0` | 정지 경로에서 전방 clear 유지 후 GLOBAL 복귀 시간 |
| `stopped_path_speed_threshold_mps` | `0.01` | 정지 waypoint 판정 속도 |
| `stopped_path_obstacle_lookahead_m` | `3.0` | 전방 장애물 확인 거리 |
| `stopped_path_ego_half_width_m` | `0.16` | 전방 corridor 횡반폭 |
