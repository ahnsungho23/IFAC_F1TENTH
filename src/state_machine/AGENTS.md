...
- Transition logic is a `committed_state_`-based FSM (dwell and safety fallback were removed 2026-07-13):
  - `local_path_confirmed()` / `can_enter_avoid()` / `can_enter_overtake()`: GLOBAL→AVOID /
    ...
    `allow_avoid_transition` / `allow_overtake_transition` (code default true; YAML: avoid true,
    overtake false) short-circuit these gates before the M-of-N check, so entry is fully blocked.
    They gate entry only — AVOID/OVERTAKE→GLOBAL merge-back ignores them. Both flags are
    DYNAMIC: an `add_on_set_parameters_callback` updates the cached members at runtime
    (map_creator lowers `allow_avoid_transition` after a successful global-line swap and restores
    it on baseline rollback). All other parameters keep read-once-at-startup semantics.
    `publish_state_cycle()` also drops disabled sources from the input warning.
