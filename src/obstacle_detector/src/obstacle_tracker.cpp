...
{
    // Called when the Frenet reference is rebuilt: existing tracks live in the old s-domain and
...
int ObstacleTracker::requiredConfirmFrames(const Track &t) const
{
    // Linear interpolation on the last measured range: close obstacles confirm after
    // confirm_frames_near consecutive matches, far ones only after confirm_frames_far.
    const double ratio = p_.confirm_range_max > 1e-6 ?
        std::clamp(t.last_range / p_.confirm_range_max, 0.0, 1.0) : 0.0;
    const double required = static_cast<double>(p_.confirm_frames_near) +
        static_cast<double>(p_.confirm_frames_far - p_.confirm_frames_near) * ratio;
    return std::max(1, static_cast<int>(std::lround(required)));
}
    ...
    Track &t, double ego_yaw_rate, bool yaw_rate_fresh) const
{
    ...
    // A track stays hidden until it matches on enough CONSECUTIVE frames (range-scaled by
    // requiredConfirmFrames). The confirming observation publishes the track immediately as
    // provisional static, then later observations collect motion evidence.
    if (!t.classified)
    {
        ...
        if (t.is_visible && t.consecutive_hits >= requiredConfirmFrames(t))
        {
            t.classified = true;
    ...
    double ego_yaw_rate, bool yaw_rate_fresh)
{
    ...
    for (std::size_t ti = 0; ti < nt; ++ti)
    {
        ...
        if (trk_assigned[ti] >= 0)
        {
            ...
            // resetting the streak; stable physical obstacles reach the publish gate within the
            // confirmation frames.
        ...
        {
            t.ttl--;
            // Prediction-only frame: both confirmation evidence and envelope stability require
            // CONSECUTIVE matched frames, so an intermittent flicker never confirms or publishes.
    ...
    for (std::size_t di = 0; di < nd; ++di)
    {
        ...
        t.last_range = detections[di].range;
        t.ttl = p_.ttl_static;
        t.is_static = true;
        t.is_visible = true;
