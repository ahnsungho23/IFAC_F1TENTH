...
랩 1에서 확정된 정적 장애물을 글로벌 경로에 **영구 반영**하는 파이프라인 노드입니다.
랩 1→2 전이 시점에 장애물 원장을 freeze하고, local_planning의 좌/우 판정 로직
...
IDLE ──(lap_count ≥ trigger, 원장 freeze)──▶ 판정+페인팅+저장 ──▶ GENERATING
  ...
  [스왑 전제] ① 생성·검증 완료  ② lap_count 다음 갱신
  [안전 게이트] STATE_GLOBAL + ego s ∈ [1,8] m ──▶ reload 호출 ──▶ MONITORING
...
5. **재생성**: 드라이버가 GUI와 동일한 `load_gui_params(gui_params.yaml)` 값으로
   ...
   게이트 실패 시 `--safety-width 0.5`로 1회 재시도.
6. **스왑**: republisher가 같은 디렉터리를 재독·검증 후 bundle 교체 + 즉시 발행.

## 3. 구독·발행·서비스

| 방향 | 이름 | 타입 | 용도 |
| --- | --- | --- | --- |
| 구독 | `/static_obs` | `f110_msgs/ObstacleArray` | 랩1 장애물 원장 |
| 구독 | `/global_waypoints` | `f110_msgs/WpntArray` | 판정 기준선 (최초 1회 = 불변 P0) |
| 구독 | `/lap_count` | `std_msgs/Int32` | 트리거·스왑 랩 경계 |
| 구독 | `/state` | `f110_msgs/StateMachine` | 스왑 게이트 (STATE_GLOBAL) |
| 구독 | `/car_state/frenet/odom` | `nav_msgs/Odometry` | 스왑 게이트 (ego s 창) |
...
| 파라미터 | 기본값 | 설명 |
| --- | ---: | --- |
| `trigger_lap_count` | 1 | freeze 트리거 랩 (랩1 완주 = 0→1) |
...
| `swap_s_window_min/max_m` | 1.0 / 8.0 | 스왑 허용 ego s 창 |
| `max_swap_deferral_laps` | 3 | 게이트 불충족 이월 상한 |
...
ros2 launch map_creator map_creator.launch.py
```

전제: global_planning(리로드 서비스 포함)·state_machine(lap_counter 포함)·
obstacle_detector가 함께 떠 있어야 하며, `offline_trajectory_generator/output/obstacle_map/`
...
1. 랩 1 주행 중 `/map_creator/status`가 `idle`인지, `/static_obs` 수신을 확인합니다.
2. 랩 1 완주 시 status가 `generating`으로 바뀌고 `regen_log.txt`가 자라는지 확인합니다.
3. `gate_report.json`의 모든 게이트 PASS 후 status `armed`를 확인합니다.
4. 다음 랩 경계(STATE_GLOBAL, s∈[1,8])에서 `swapped: reloaded N waypoints ...`와
   RViz의 글로벌 라인이 장애물 밖으로 이동하는 것을 확인합니다.
5. 랩 2+에서 로컬 플래너가 해당 장애물에 재개입하지 않는지 확인합니다.
