...
def load_map_from_bag(bag):
    """백의 /map 을 그대로 쓴다.

    주행에 쓴 맵이 저장소에 없을 수 있고(08-18 22:39 이 그랬다), png 왕복은 상하 반전
    규약에서 오류가 나기 쉽다. 백에 /map 이 있으면 이 경로가 가장 안전하다.
    """
    import numpy as _np
    for _topic, m, _ts in read_bag(bag, ["/map"]):
        g = _np.asarray(m.data, dtype=_np.int16).reshape(m.info.height, m.info.width)
        occupied = g >= 50
        return occupied, ~occupied, m.info.resolution, \
               m.info.origin.position.x, m.info.origin.position.y
    raise SystemExit(f"{bag}: /map 이 없습니다. --map 으로 yaml 을 지정하십시오.")
...
def load_map(map_yaml):
    ...
    # 노드와 같은 기준을 쓴다: map_occupied_thresh 이상만 점유, 나머지는 전부 주행가능.
    # 미지(unknown)를 free 에서 빼면 안 된다 — 08-18 22:39 SLAM 맵은 값이 unknown 과
    # occupied 뿐이라 free 가 0개가 되고 거리장이 통째로 망가진다(잔차가 7 m 로 나왔다).
    del free_th
...
def score(bag, map_yaml, span, max_range, max_scans, laser_x, autonomous_only):
    if map_yaml == "BAG":
        occupied, free, res, ox, oy = load_map_from_bag(bag)
    else:
...
def main():
    ...
    ap.add_argument("--map", default=DEFAULT_MAP,
                    help="맵 yaml 경로, 또는 BAG (백에 든 /map 을 그대로 사용)")
