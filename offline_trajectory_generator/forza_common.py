#!/usr/bin/env python3
"""Shared core for the ForzaETH offline trajectory generator (Python only).

Forza-only module: the C++ pipeline (src/trajectory_core.cpp,
bin/generate_global_trajectory, trajectory_gui.py) does not import anything from
here, and nothing here calls into it. Copied verbatim from edge_test@95843f7a's
generate_global_trajectory.py, with one change: write_outputs() now emits the
RViz marker arrays instead of hard-coded empty ones (see the marker section).

Parts of this file are dead weight for the Forza path (the `laptime` and
shortest-path optimizers were superseded by the C++ generator). They are kept
so the copy stays byte-comparable with the original.
"""
...
def add_generator_arguments(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        ...
        help="SLAM map YAML path (map_server format). Required unless --map-kissmap is given.",
    )
    parser.add_argument(
        "--map-kissmap", type=Path, default=None,
        help="kinematic_localization .kissmap point map. It is rasterized to a "
             "map_server PGM/YAML pair next to the output and then fed through the "
             "normal pipeline, so the raceline is built against exactly the geometry "
             "the localizer registers to. Mutually exclusive with --map-yaml.",
    )
    parser.add_argument(
        "--kissmap-resolution", type=float, default=0.05,
        help="Cell size [m] of the occupancy grid rasterized from the .kissmap.",
    )
    parser.add_argument(
        "--kissmap-wall-seal", type=float, default=0.0,
        help="Morphological closing radius [m] used to make the wall loop watertight. "
             "0 = auto: start at the p99 nearest-neighbour point spacing and grow one "
             "cell at a time until an enclosed region appears (up to --kissmap-max-seal). "
             "Closing restores a straight wall's original thickness, so this does NOT "
             "narrow the corridor — but it does round off free-space corners tighter "
             "than half the radius.",
    )
    parser.add_argument(
        "--kissmap-max-seal", type=float, default=1.0,
        help="Upper bound [m] for the automatic wall-seal search. Hitting it means the "
             "wall loop has a hole wider than this and the map needs re-recording.",
    )
    parser.add_argument(
        "--kissmap-margin", type=float, default=0.5,
        help="Free border [m] added around the point bounding box. The border ring is "
             "what the outside-flood-fill starts from, so keep it >= one cell.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    parser.add_argument("--waypoint-step", type=float, default=0.25, help="Final waypoint spacing [m].")
    parser.add_argument("--optimizer-step", type=float, default=0.46, help="Internal optimizer spacing [m].")
    parser.add_argument("--safety-width", type=float, default=0.4, help="Vehicle+safety width used for bounds [m].")
    parser.add_argument(
        "--boundary-margin", type=float, default=0.6,
        help="Extra margin from detected walls [m]. Together with --safety-width it sets the\n"
             "raceline corridor: clearance = 0.5*safety_width + boundary_margin.")
    parser.add_argument("--max-width-distance", type=float, default=2.4, help="Raycast limit for track width [m].")
    parser.add_argument(
        "--width-mode",
    ...
    parser.add_argument("--max-speed", type=float, default=8.0, help="Maximum waypoint speed [m/s].")
    parser.add_argument("--min-speed", type=float, default=3.4, help="Minimum waypoint speed [m/s].")
    parser.add_argument(
        "--velocity-limits-csv",
        type=Path,
    ...
    parser.add_argument(
        "--max-curvature", type=float, default=0.7,
    ...
    parser.add_argument("--smooth-sigma", type=float, default=4.1, help="Closed-curve smoothing sigma in samples.")
    parser.add_argument(
    ...
    parser.add_argument("--median-kernel", type=int, default=5, help="Odd pixel kernel for salt-and-pepper map denoising.")
    parser.add_argument("--morph-kernel", type=int, default=5, help="Map cleanup kernel size in pixels.")
    parser.add_argument("--morph-open-iterations", type=int, default=4)
    parser.add_argument("--morph-close-iterations", type=int, default=1)
    parser.add_argument("--skeleton-prune-iterations", type=int, default=110)
    ...
    parser.add_argument("--max-optimizer-iter", type=int, default=370)
    parser.add_argument("--curvature-weight", type=float, default=0.6)
    parser.add_argument("--smooth-weight", type=float, default=0.095)
...
def default_output_dir(map_path: Path) -> Path:
    return Path(__file__).resolve().parent / "output" / Path(map_path).stem


def resolve_output_dir(args: argparse.Namespace) -> Path:
    """Output directory for this run, named after whichever map input was given."""
    if getattr(args, "output_dir", None) is not None:
        return Path(args.output_dir)
    source = getattr(args, "map_kissmap", None) or args.map_yaml
    return default_output_dir(Path(source))


def validate_args(args: argparse.Namespace) -> None:
    map_yaml = getattr(args, "map_yaml", None)
    map_kissmap = getattr(args, "map_kissmap", None)
    if (map_yaml is None) == (map_kissmap is None):
        raise RuntimeError("Give exactly one of --map-yaml or --map-kissmap.")
    if map_kissmap is not None:
        if args.kissmap_resolution <= 0.0:
            raise RuntimeError("kissmap-resolution must be positive.")
        if args.kissmap_wall_seal < 0.0:
            raise RuntimeError("kissmap-wall-seal must be non-negative (0 = auto).")
        if args.kissmap_max_seal <= 0.0:
            raise RuntimeError("kissmap-max-seal must be positive.")
        if args.kissmap_margin < args.kissmap_resolution:
            raise RuntimeError("kissmap-margin must be at least one cell.")
...
# .kissmap input
# ---------------------------------------------------------------------------
# `.kissmap` is the frozen point map kinematic_localization registers scans
# against (format defined in src/kinematic_localization/src/utils.hpp):
#
#   8 bytes  magic "KISSMAP1"
#   double   voxel_size
#   double   max_range
#   uint64   point count
#   double   x, y, z   x count            (little-endian, map frame)
#
# It only ever states "a surface was measured here" — there is no free/unknown
# information at all. So the conversion has to invent free space, and the one
# assumption that is always true for a closed track is: the drivable area is the
# region enclosed by the walls, i.e. everything that the outside cannot reach.
# That is a border flood fill, not a threshold, which is why this cannot reuse
# load_map()'s trinary logic directly.
KISSMAP_MAGIC = b"KISSMAP1"
KISSMAP_FREE_PIXEL = 254
KISSMAP_UNKNOWN_PIXEL = 205
KISSMAP_OCCUPIED_PIXEL = 0


def read_kissmap(path: Path) -> tuple[np.ndarray, float, float]:
    """Return (points_xy [N,2], voxel_size, max_range) from a .kissmap file."""
    raw = path.read_bytes()
    header = 8 + 8 + 8 + 8
    if len(raw) < header or raw[:8] != KISSMAP_MAGIC:
        raise RuntimeError(f"{path} is not a KISSMAP1 file.")
    voxel_size, max_range = np.frombuffer(raw, dtype="<f8", count=2, offset=8)
    count = int(np.frombuffer(raw, dtype="<u8", count=1, offset=24)[0])
    expected = header + count * 3 * 8
    if len(raw) < expected:
        raise RuntimeError(
            f"{path} is truncated: header claims {count} points "
            f"({expected} bytes) but the file is {len(raw)} bytes."
        )
    points = np.frombuffer(raw, dtype="<f8", count=count * 3, offset=header).reshape(count, 3)
    if count < 3:
        raise RuntimeError(f"{path} holds {count} points — not enough to form a track.")
    return np.ascontiguousarray(points[:, :2]), float(voxel_size), float(max_range)


def kissmap_point_gap(points_xy: np.ndarray) -> float:
    """Largest nearest-neighbour distance between wall points [m].

    The stored points sit on a scan-derived grid (0.07-0.25 m in the maps this repo
    ships), so painting one cell per point leaves the wall as a dotted line and a flood
    fill leaks straight through it. Dilating by this distance provably bridges every
    such gap (a gap g closes at radius g/2) with a factor of two to spare, which the
    regrow step in `kissmap_to_map_files` needs — see `seal_disk`.

    The MAXIMUM is used, not a percentile: one unbridged gap anywhere is enough to let
    the fill escape, and on all three maps in this repo the max sits within 1.1-2.7x of
    the median, so it is not outlier-driven. Override with --kissmap-wall-seal if a map
    ever carries a genuinely isolated stray point.
    """
    from scipy.spatial import cKDTree

    tree = cKDTree(points_xy)
    # k=2 because the nearest neighbour of a point is itself.
    spacing = tree.query(points_xy, k=2)[0][:, 1]
    spacing = spacing[np.isfinite(spacing)]
    return 0.0 if spacing.size == 0 else float(np.max(spacing))


def seal_disk(seal_px: int) -> np.ndarray:
    return cv2.getStructuringElement(
        cv2.MORPH_ELLIPSE, (2 * max(1, seal_px) + 1, 2 * max(1, seal_px) + 1)
    )


def seal_walls(occupied: np.ndarray, seal_px: int) -> np.ndarray:
    """Thicken the wall until the dotted line is watertight.

    ⚠️ This must be a DILATION, not a closing. Closing (dilate then erode) preserves a
    straight wall's thickness, which is why it looks like the tidier choice — but that
    same property means it CANNOT bridge a gap between two thin collinear segments: the
    dilation's waist over a gap g is only sqrt(R^2 - (g/2)^2) high, and the erosion that
    follows removes anything thinner than the full 2R. Measured on the real Jetson map
    (1,536 points, 0.10 m spacing): closing produced ZERO enclosed rings at every radius
    from 0.05 m to 1.00 m, while dilation sealed it at the first cell.

    The corridor loses `seal_px` cells on each side here; `kissmap_to_map_files` grows it
    back with the same disk afterwards, so the published track widths stay honest.
    """
    return cv2.dilate(occupied, seal_disk(seal_px))


def enclosed_mask(occupied: np.ndarray) -> np.ndarray:
    """Cells the image border cannot reach without crossing a wall.

    A `.kissmap` only ever states "a surface was measured here" — it carries no
    free/unknown information at all. The one assumption that always holds for a
    closed track is that the drivable area is the region the outside cannot reach,
    so free space is recovered by a border flood fill rather than a threshold.
    """
    height, width = occupied.shape
    flood = (occupied == 0).astype(np.uint8)
    if flood[0, 0] == 0:
        raise RuntimeError(
            "The .kissmap border cell is already occupied — raise --kissmap-margin."
        )
    cv2.floodFill(flood, np.zeros((height + 2, width + 2), dtype=np.uint8), (0, 0), 2)
    return flood == 1


def track_component(enclosed: np.ndarray) -> np.ndarray:
    """Pick the drivable annulus out of the enclosed regions.

    "Enclosed by walls" alone is not enough: the interior of every infield island is
    enclosed too, and on the maps in this repo an island interior is often the LARGEST
    enclosed blob, so "take the biggest" picks the infield and the generator then
    extracts a centerline that never touches the track. What separates the two is
    topology — a closed circuit is a RING (it wraps around its infield, so it has at
    least one hole), an island interior is simply connected. Verified on the
    2026-08-19 21:45 real-car map: the annulus came out as 86.3 m^2 with a 30.3 m^2
    hole while the three island interiors (12.3 / 10.5 / 7.5 m^2) had none.
    """
    if not bool(np.any(enclosed)):
        return np.zeros_like(enclosed, dtype=bool)
    ...
    for label in range(1, num_labels):
        ...
        if not bool(np.any(ndimage.binary_fill_holes(component) & ~component)):
            continue                       # no hole -> island interior, not a circuit
        area = int(stats[label, cv2.CC_STAT_AREA])
        if area > best_area:
            best_area = area
            best = component
    return np.zeros_like(enclosed, dtype=bool) if best is None else best


# An enclosed pocket smaller than this is LiDAR salt noise, not a track: a corridor
# one car long is already ~0.5 x 0.3 m, and any real lap encloses two orders of
# magnitude more than this.
KISSMAP_MIN_TRACK_AREA_M2 = 1.0
def kissmap_to_map_files(
    kissmap_path: Path,
    output_dir: Path,
    resolution: float,
    wall_seal_m: float,
    ...
    """Rasterize a .kissmap into a map_server PGM/YAML pair and return the YAML path.

    The pair is written for real (not held in memory) so the operator can open it in
    RViz, re-run the generator on it, and diff it against the SLAM map.

    Pipeline: paint one cell per point -> dilate by `seal` so the dotted wall is
    watertight -> border flood fill -> keep the ring-shaped enclosed region -> dilate
    that region back by the SAME `seal` so the corridor regains the width the sealing
    took from it -> subtract the measured wall cells.
    """
    points, voxel_size, _ = read_kissmap(kissmap_path)
    gap_m = kissmap_point_gap(points)

    if wall_seal_m > 0.0:
        seal_candidates_px = [max(1, int(round(wall_seal_m / resolution)))]
    else:
        # Start at the largest point-to-point gap. A gap g is bridged by a dilation of
        # radius g/2, so this is twice what sealing alone needs — the surplus is what
        # keeps the regrow step from squeezing back out through the same gap (the
        # regrow reaches R - sqrt(R^2 - (g/2)^2) past the wall line, which is 9 mm at
        # R = g = 0.14 m). Grow one cell at a time from there: real SLAM maps also have
        # genuine holes (a corner the LiDAR never saw) that need more than the dots do.
        first = max(1, int(math.ceil(max(gap_m, resolution) / resolution)))
        last = max(first, int(round(max_seal_m / resolution)))
        seal_candidates_px = list(range(first, last + 1))

    # The border ring must stay free even after the widest dilation, otherwise the flood
    # fill has no seed. Grow the margin rather than failing.
    margin = max(margin_m, max(seal_candidates_px) * resolution + 2.0 * resolution)
    origin_x = float(points[:, 0].min()) - margin
    origin_y = float(points[:, 1].min()) - margin
    width = int(math.ceil((float(points[:, 0].max()) - origin_x + margin) / resolution)) + 1
    height = int(math.ceil((float(points[:, 1].max()) - origin_y + margin) / resolution)) + 1

    painted = np.zeros((height, width), dtype=np.uint8)
    cols = np.clip(((points[:, 0] - origin_x) / resolution).astype(int), 0, width - 1)
    rows = np.clip(((points[:, 1] - origin_y) / resolution).astype(int), 0, height - 1)
    painted[rows, cols] = 1

    seal_px = 0
    inside = np.zeros((height, width), dtype=bool)
    for candidate_px in seal_candidates_px:
        ring = track_component(enclosed_mask(seal_walls(painted, candidate_px)))
        if not bool(np.any(ring)):
            continue
        # Give back exactly what the sealing dilation took: without this the published
        # d_left/d_right would be understated by `seal` on BOTH sides, which is the very
        # error this whole change exists to remove.
        inside = (
            cv2.dilate(ring.astype(np.uint8), seal_disk(candidate_px)) > 0
        ) & (painted == 0)
        seal_px = candidate_px
        break

    free_area = float(np.count_nonzero(inside)) * resolution * resolution
    if free_area < KISSMAP_MIN_TRACK_AREA_M2:
        raise RuntimeError(
            f"No closed circuit was found in {kissmap_path.name} up to a wall seal of "
            f"{seal_candidates_px[-1] * resolution:.2f} m. Either the outer wall has a "
            "gap wider than that — raise --kissmap-max-seal or re-record the missing "
            "stretch — or the recording covers only part of the loop. Rasterize it once "
            "with --kissmap-wall-seal set by hand and open the generated PGM to see "
            "where the boundary is broken."
        )
    seal_m = seal_px * resolution
    # Compare against the FIRST candidate, not gap_m: the first candidate is gap_m
    # rounded up to a whole cell, so comparing to gap_m fires this note on every map.
    if wall_seal_m <= 0.0 and seal_px > seal_candidates_px[0]:
        print(
            f"[kissmap] NOTE: sealing the wall needed {seal_m:.2f} m but the largest gap "
            f"between neighbouring points is only {gap_m:.2f} m — the map has real holes, "
            "not just dotted walls. Free-space corners tighter than the seal radius are "
            "rounded off by the regrow; check the generated PGM before trusting the "
            "corridor width."
        )

    image = np.full((height, width), KISSMAP_UNKNOWN_PIXEL, dtype=np.uint8)
    image[inside] = KISSMAP_FREE_PIXEL
    # The measured points are the wall — not the sealing dilation around them.
    image[painted > 0] = KISSMAP_OCCUPIED_PIXEL
    # map_server images store row 0 as the TOP row (max y); the grid above is built
    # bottom-up, so flip before writing (matches pixel_to_world's flip_y).
    image = np.flipud(image)

    output_dir.mkdir(parents=True, exist_ok=True)
    stem = f"{kissmap_path.stem}_kissmap"
    image_path = output_dir / f"{stem}.pgm"
    yaml_path = output_dir / f"{stem}.yaml"
    if not cv2.imwrite(str(image_path), image):
        raise RuntimeError(f"Could not write {image_path}")
    yaml_path.write_text(
        f"# Generated from {kissmap_path} by generate_global_trajectory.py\n"
        f"# kissmap voxel_size: {voxel_size:.3f} m, points: {len(points)}, "
        f"largest point gap: {gap_m:.3f} m, wall seal: {seal_m:.3f} m\n"
    ...
    print(
        f"[kissmap] {kissmap_path.name}: {len(points)} points -> {width}x{height} @ "
        f"{resolution:.3f} m, largest point gap {gap_m:.3f} m, wall seal {seal_m:.3f} m, "
        f"drivable area {free_area:.1f} m^2"
    )
    print(f"[kissmap] Wrote {yaml_path} / {image_path.name}")
    return yaml_path
...
# ---------------------------------------------------------------------------
# RViz marker payloads. Ported 1:1 from trajectory_core.cpp:1125~1230 so the
# JSON written here is a drop-in source for global_trajectory_publisher_node.
# That node publishes a single DELETEALL when a marker array arrives empty, so
# omitting these does not merely hide the raceline -- it wipes the previous
# map's markers from RViz as well.
...
def trajectory_markers(traj: Trajectory) -> dict:
    ...
    for i in range(n):
        marker["points"].append(marker_point(traj.points_xy[i][0], traj.points_xy[i][1]))
        ratio = min(max((float(traj.vx_mps[i]) - vmin) / span, 0.0), 1.0)
        marker["colors"].append(marker_color(ratio, 1.0 - ratio, 0.0))
...
def trackbound_markers(traj: Trajectory) -> dict:
    ...
    left = line_strip("trackbound_left", 0, TRACKBOUND_MARKER_WIDTH, marker_color(0.2, 0.6, 1.0))
    right = line_strip("trackbound_right", 1, TRACKBOUND_MARKER_WIDTH, marker_color(0.2, 0.6, 1.0))
    for i in range(n):
        sin_psi = math.sin(float(traj.psi_rad[i]))
        cos_psi = math.cos(float(traj.psi_rad[i]))
        x = float(traj.points_xy[i][0])
        y = float(traj.points_xy[i][1])
        left["points"].append(
            marker_point(x - traj.d_left[i] * sin_psi, y + traj.d_left[i] * cos_psi)
        )
        right["points"].append(
            marker_point(x + traj.d_right[i] * sin_psi, y - traj.d_right[i] * cos_psi)
        )
    if n > 2:
...
def centerline_markers(traj: Trajectory) -> dict:
    ...
    marker = line_strip("centerline", 0, TRACKBOUND_MARKER_WIDTH, marker_color(0.5, 0.5, 0.5))
    for i in range(n):
        marker["points"].append(marker_point(traj.points_xy[i][0], traj.points_xy[i][1]))
    if n > 2:
        marker["points"].append(marker["points"][0])
...
def write_outputs(
    ...
    payload = {
        ...
        "global_traj_markers_sp": empty_marker_array(),  # Forza generates no shortest path
...
def generate_trajectory(args: argparse.Namespace) -> GenerationResult:
    validate_args(args)
    if getattr(args, "map_kissmap", None) is not None:
        # Rasterize first, then run the ordinary map pipeline on the result — the
        # generated PGM/YAML is also the artifact the operator inspects in RViz.
        args.map_yaml = kissmap_to_map_files(
            Path(args.map_kissmap),
            resolve_output_dir(args),
            args.kissmap_resolution,
            args.kissmap_wall_seal,
            args.kissmap_margin,
            args.kissmap_max_seal,
        )
...
def main() -> int:
    args = parse_args()
    # Resolved before generation because generate_trajectory() rewrites args.map_yaml
    # when the input is a .kissmap.
